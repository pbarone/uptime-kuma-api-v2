...
Two defects, each found by live verification and each present in the original library identically: on Uptime Kuma 2.x the cached monitor list went stale after every monitor mutation (found during 2.3.0 verification against server 2.4.0), and on **every Uptime Kuma 1.x server `add_monitor()` failed outright** because the 2.x-only `conditions` field was sent ungated (found while verifying that first fix against a 1.23.2 server). No new public API surface for either. The 2.x cache fix added and changed no version gating; the 1.x fix adds gating to eight monitor fields that had none, and leaves every gate that already existed exactly as it was.
